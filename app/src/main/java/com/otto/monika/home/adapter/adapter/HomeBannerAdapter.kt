...
class HomeBannerAdapter(
    val onItemClick: ((MonikaBannerItem) -> Unit)? = null
) : RecyclerView.Adapter<HomeBannerAdapter.Holder>() {

    var bannerData: MonikaBannerData? = null
        set(value) {
            field = value
            notifyItemChanged(0)
        }
    ...
    override fun onCreateViewHolder(
        ...
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_monika_fragment_banner_view, parent, false)
        return Holder(view)
    }
    ...
    override fun onBindViewHolder(
        ...
        bannerData?.list?.let { bannerList.addAll(it) }
        holder.itemView.isVisible = bannerList.isNotEmpty()
        if (bannerList.isEmpty())
            return
        val screenWidth = DipUtils.getScreenWidth(holder.recyclerView.context)
        // 设置卡片宽度：中间选中卡片的宽度
        ...
        val recyclerAdapter = BannerRecyclerAdapter(holder.recyclerView.context, bannerList)
        val layoutManager =
